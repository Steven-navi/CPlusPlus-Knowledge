<font size=4>编译内存相关</font>

***

本章是C++面经的开章之作，其中涉及C++程序的编译过程、内存以及头文件的一些知识，重点在内存方面进行展开，包括内存的分区、内存对齐、内存泄漏、内存泄漏的防止方法、现有的检测内存泄漏的工具等等。由于问题之间的关联性，可能有些问题并非是本章相关的知识点，例如一些问题涉及到了类中的虚函数、创建类的对象的底层原理等等，但为了保持问题上下的连贯性，也放在了本章，便于问题的理解。



<font color=blue size=5>A</font>   <font size=4>C++程序编译过程</font>
...
![image-20210716145842640](C:\Users\Administrator\AppData\Roaming\Typora\typora-user-images\image-20210716145842640.png)
...
<font color=blue size=5>A</font>   <font size=4>C++内存管理</font>
...
- 从操作系统本身来讲，以上存储区在内存中的分布是如下形式(从低地址到高地址)：.text 段 --> .data 段 --> .bss 段 --> 堆 --> unuesd --> 栈 --> env
...
<font color=blue size=5>A</font>   <font size=4>堆和栈的区别</font>
...
<font color=blue size=5>A</font>   <font size=4>变量的区别</font>
...
- **全局变量**：具有全局作用域。全局变量只需在一个源文件中定义，就可以作用于所有的源文件。当然，其他不包含全局变量的源文件需要用 `extern` 关键字再次声明这个全局变量。
- **静态全局变量**：具有文件作用域。它与全局变量的区别在于如果程序包含多个文件的话，它作用于定义它的文件里，不能作用到其他文件里，即被 static 关键字修饰过的变量具有文件作用域。这样即使两个不同的源文件都定义了相同名字的静态全局变量，它们也是不同的变量。
...
<font color=blue size=5>A</font>   <font size=4>全局变量定义在头文件中有什么问题？</font>

面试高频指数：★☆☆☆☆

***

如果在头文件中定义全局变量，当该文件被多个文件 include 时，该头文件中的全局变量就会被定义多次，导致重复定义，因此不能在头文件中定义全局变量。



<font color=blue size=5>A</font>   <font size=4>对象创建限制在堆或栈</font>
...
- 静态建立：由编译器为对象在栈上分配内存，直接调用类的构造函数创建对象。例如：`A a;`
...
<font color=blue size=5>A</font>   <font size=4>内存对齐</font>

面试高频指数：★★☆☆☆

***

**什么是内存对齐？内存对齐的原则？为什么要进行内存对齐，有什么有点？**

内存对齐：编译器将程序中的每一个”数据单元“安排在字的整数倍的地址指向的内存中
...
<font color=blue size=5>A</font>   <font size=4>类的大小</font>
...
<font color=blue size=5>A</font>   <font size=4>什么是内存泄漏</font>
...
- 使用 `malloc`、`calloc`、`realloc`、`new` 等分配内存时，使用完后要调用相应的 `free` 或 `delete` 释放内存，否则这块内存就会造成内存泄
...
<font color=blue size=5>A</font>   <font size=4>怎么防止内存泄漏？内存泄漏检测工具的原理？</font>
...
2. 智能指针
   ...
   智能指针是C++中已经对内存泄漏封装好了一个工具，可以直接拿来使用，将在下一个问题中对智能指针进行详细的解释。

   **内存泄漏检测工具的实现原理：**

   内存检测工具有很多，这里重点介绍下 valgrind 。

   ![image-20210718141056938](C:\Users\Administrator\AppData\Roaming\Typora\typora-user-images\image-20210718141056938.png)
...
- 当要读写内存中某个字节时，首先检查这个字节对应的 Vaild-Address 表中对应的 bit。如果该 bit 显示该位置是无效位置，Memcheck 则报告读写错误。
- 内核（core）类似于一个虚拟的 CPU 环境，这样当内存中的某个字节被加载到真实的 CPU 中时，该字节在 Vaild-Value 表对应的 bits 也被加载到虚拟的 CPU 环境中。一旦寄存器中的值，被用来产生内存地址，或者该值能够影响程序输出，则 Memcheck 会检查 Valid-Value 表对应的 bits，如果该值尚未初始化，则会报告使用未初始化内存错误。



<font color=blue size=5>A</font>   <font size=4>智能指针有哪几种？智能指针的实现原理？</font>
...
<font color=blue size=5>A</font>   <font size=4>一个 unique_ptr 怎么赋值给另一个 unique_ptr 对象？</font>
...
<font color=blue size=5>A</font>   <font size=4>使用智能指针会出现什么问题？怎么解决？</font>
...
<font color=blue size=5>A</font>   <font size=4>C++ 11 新特性</font>

面试高频指数：★★★★☆

***

说明：C++11的新特性有很多，从面试角度来讲，如果面试官问道该问题，常以该问题作为引子，对面试者提到的知识点进行深入展开提问。面试者尽可能的列举常用的并且熟悉的特性，尽可能的掌握相关原理，下文只是对相关知识点进行了简单的阐述，有关细节还需要结合相关知识点的相关问题。

下面对常用的做下总结：

1. `auto`类型推导

`auto`关键字：自动类型推导，编译器会在 **编译期间** 通过初始初值推导出变量的类型，通过 `auto` 定义的变量必须有初值。
...
2. `decltype` 类型推导
   ...
   `decltype` 关键字：`decltype` 是"declare type"的缩写，译为”声明类型“。和 `auto` 的功能一样，都用来在编译时期进行自动类型推导。如果希望从表达式中推断出要定义的变量的类型，但不想用该表达式的值初始化变量，这时就不能再用 `auto` 。`decltype`  作用是选择并返回操作数的数据类型。
...
<font color=blue size=5>A</font>   <font size=4>C 和 C++ 的区别</font>
...
<font color=blue size=5>A</font>   <font size=4>Java 和 C++ 的区别</font>
...
<font color=blue size=5>A</font>   <font size=4>Python 和 C++ 的区别</font>
...
<font color=blue size=5>A</font>   <font size=4>什么是面向对象？面向对象的三大特性</font>
...
- 封装：将具体的实现过程和数据封装成一个函数，只能通过接口进行访问，降低耦合特性。
- 继承：子类继承父类的特征和行为，子类有父类的非 `private` 方法或成员变量，子类可以对父类的方法进行重写，增强了类之间的耦合性，但是当父类中的成员变量、成员函数或者类本身被 `final` 关键字修饰时，修饰的类不能继承，修饰的成员不能重写或修改。
- 多态：多态就是不同继承类的对象，对同一消息做出不同的响应，基类的指针指向或绑定到派生类的对象，使得基类指针呈现不同的表现方式。



<font color=blue size=5>A</font>   <font size=4>重载、重写、隐藏的区别</font>
...
<font color=blue size=5>A</font>   <font size=4>如何理解 C++ 是面向对象编程</font>
...
<font color=blue size=5>A</font>   <font size=4>什么是多态？多态如何实现？</font>
...
![image-20210718222140295](C:\Users\Administrator\AppData\Roaming\Typora\typora-user-images\image-20210718222140295.png)

派生类的对象虚函数表如下：

![image-20210718222302495](C:\Users\Administrator\AppData\Roaming\Typora\typora-user-images\image-20210718222302495.png)
...
<font color=blue size=5>A</font>   <font size=4>sizeof 和 strlen 的区别</font>
...
<font color=blue size=5>A</font>   <font size=4>lambda 表达式（匿名函数）的具体应用和使用场景</font>
...
<font color=blue size=5>A</font>   <font size=4>explicit 的作用（如何避免编译器进行隐式类型转换）</font>
...
<font color=blue size=5>A</font>   <font size=4>C 和 C++ static 的区别</font>
...
<font color=blue size=5>A</font>   <font size=4>static 的作用</font>
...
<font color=blue size=5>A</font>   <font size=4>static 在类中使用的注意事项（定义、初始化和使用）</font>
...
<font color=blue size=5>A</font>   <font size=4>static 全局变量和普通全局变量的异同</font>
...
<font color=blue size=5>A</font>   <font size=4>const 作用及用法</font>
...
<font color=blue size=5>A</font>   <font size=4>define 和 const 的区别</font>
...
<font color=blue size=5>A</font>   <font size=4>define 和 typedef 的区别</font>
...
<font color=blue size=5>A</font>   <font size=4>用宏实现比较大小，以及两个数中的最小值</font>
...
<font color=blue size=5>A</font>   <font size=4>inline 作用及使用方法</font>
...
另外，可以在声明函数和定义函数的同时加上 `inline`；也可以只在函数声明时加 `inline`，而定义函数时不加 `inline`。另外，可以在声明函数和定义函数的同时加上 `inline`；也可以只在函数声明时加 `inline`，而定义函数时不加 `inline`。



<font color=blue size=5>A</font>   <font size=4>inline 函数工作原理</font>
...
<font color=blue size=5>A</font>   <font size=4>宏定义（define）和内联函数（inline）的区别</font>
...
<font color=blue size=5>A</font>   <font size=4>new 的作用？</font>
...
<font color=blue size=5>A</font>   <font size=4>new 和 malloc 如何判断是否申请到内存？</font>
...
<font color=blue size=5>A</font>   <font size=4>delete 实现原理？delete 和 delete[] 的区别？</font>
...
<font color=blue size=5>A</font>   <font size=4>new 和 malloc 的区别，delete 和 free 的区别</font>
...
<font color=blue size=5>A</font>   <font size=4>malloc 的原理？malloc 的底层实现？</font>
...
<font color=blue size=5>A</font>   <font size=4>C 和 C++ struct 的区别？</font>
...
<font color=blue size=5>A</font>   <font size=4>为什么有了 class 还保留 struct？</font>
...
<font color=blue size=5>A</font>   <font size=4>struct 和 union 的区别</font>
...
<font color=blue size=5>A</font>   <font size=4>class 和 struct 的异同</font>
...
<font color=blue size=5>A</font>   <font size=4>volatile 的作用？是否具有原子性，对编译器有什么影响？</font>
...
<font color=blue size=5>A</font>   <font size=4>什么情况下一定要用 volatile， 能否和 const 一起使用？</font>
...
<font color=blue size=5>A</font>   <font size=4>返回函数中静态变量的地址会发生什么？</font>
...
<font color=blue size=5>A</font>   <font size=4>extern C 的作用？</font>
...
<font color=blue size=5>A</font>   <font size=4>sizeof(1==1) 在 C 和 C++ 中分别是什么结果？</font>
...
<font color=blue size=5>A</font>   <font size=4>memcpy 函数的底层原理？</font>
...
<font color=blue size=5>A</font>   <font size=4>strcpy 函数有什么缺陷？</font>
...
原因：变量 `arr` 只分配的 10 个内存空间，通过上述程序中的地址可以看出 `arr` 和 `var` 在内存中是连续存放的，但是在调用`strcpy` 函数进行拷贝时，源字符串 `"hello world!"` 所占的内存空间为 13，因此在拷贝的过



<font color=blue size=5>A</font>   <font size=4>auto 类型推导的原理</font>
...
<font color=blue size=5>A</font>   <font size=4>什么是虚函数？什么是纯虚函数？</font>
...
<font color=blue size=5>A</font>   <font size=4>虚函数和纯虚函数的区别？</font>
...
<font color=blue size=5>A</font>   <font size=4>虚函数的实现机制</font>
...
![image-20210719171234228](C:\Users\Administrator\AppData\Roaming\Typora\typora-user-images\image-20210719171234228.png)

基类的虚函数表：

![image-20210719171300367](C:\Users\Administrator\AppData\Roaming\Typora\typora-user-images\image-20210719171300367.png)

派生类的虚函数表：

![image-20210719171337124](C:\Users\Administrator\AppData\Roaming\Typora\typora-user-images\image-20210719171337124.png)

主函数中基类的指针 `p` 指向了派生类的对象，当调用函数 `B_fun1()` 时，通过派生类的虚函数表找到该函数的地址，从而完成调用。



<font color=blue size=5>A</font>   <font size=4>单继承和多继承的虚函数表结构</font>
...
![image-20210719203646904](C:\Users\Administrator\AppData\Roaming\Typora\typora-user-images\image-20210719203646904.png)

基类的虚函数表：

![image-20210719203718354](C:\Users\Administrator\AppData\Roaming\Typora\typora-user-images\image-20210719203718354.png)

派生类的虚函数表：

![image-20210719203749633](C:\Users\Administrator\AppData\Roaming\Typora\typora-user-images\image-20210719203749633.png)
...
![image-20210719204008076](C:\Users\Administrator\AppData\Roaming\Typora\typora-user-images\image-20210719204008076.png)
...
![image-20210719204151373](C:\Users\Administrator\AppData\Roaming\Typora\typora-user-images\image-20210719204151373.png)

派生类的虚函数表：（基类的顺序和声明的顺序一致）

![image-20210719204242881](C:\Users\Administrator\AppData\Roaming\Typora\typora-user-images\image-20210719204242881.png)
...
![image-20210719213549809](C:\Users\Administrator\AppData\Roaming\Typora\typora-user-images\image-20210719213549809.png)

派生类的虚函数表：

![image-20210719213614029](C:\Users\Administrator\AppData\Roaming\Typora\typora-user-images\image-20210719213614029.png)



<font color=blue size=5>A</font>   <font size=4>如何禁止构造函数的使用？</font>
...
<font color=blue size=5>A</font>   <font size=4>什么是类的默认构造函数？</font>
...
<font color=blue size=5>A</font>   <font size=4>构造函数、析构函数是否需要定义成虚函数？为什么？</font>
...
<font color=blue size=5>A</font>   <font size=4>如何避免拷贝？</font>
...
<font color=blue size=5>A</font>   <font size=4>如何减少构造函数开销？</font>
...
<font color=blue size=5>A</font>   <font size=4>多重继承时会出现什么状况？如何解决？</font>
...
![image-20210719225402256](C:\Users\Administrator\AppData\Roaming\Typora\typora-user-images\image-20210719225402256.png)
...
![image-20210719233334999](C:\Users\Administrator\AppData\Roaming\Typora\typora-user-images\image-20210719233334999.png)



<font color=blue size=5>A</font>   <font size=4>空类占多少字节？C++ 编译器会给一个空类自动生成哪些函数？</font>
...
<font color=blue size=5>A</font>   <font size=4>为什么拷贝构造函数必须为引用？</font>
...
<font color=blue size=5>A</font>   <font size=4>C++ 类对象的初始化顺序</font>
...
<font color=blue size=5>A</font>   <font size=4>如何禁止一个类被实例化？</font>
...
<font color=blue size=5>A</font>   <font size=4>为什么用成员初始化列表会快一些？</font>
...
<font color=blue size=5>A</font>   <font size=4>实例化一个对象需要哪几个阶段</font>
...
<font color=blue size=5>A</font>   <font size=4>友元函数的作用及使用场景</font>
...
<font color=blue size=5>A</font>   <font size=4>静态绑定和动态绑定是怎么实现的？</font>
...
<font color=blue size=5>A</font>   <font size=4>深拷贝和浅拷贝的区别</font>
...
<font color=blue size=5>A</font>   <font size=4>编译时多态和运行时多态的区别</font>
...
<font color=blue size=5>A</font>   <font size=4>实现一个类成员函数，要求不允许修改类的成员变量？</font>
...
<font color=blue size=5>A</font>   <font size=4>如何让类不能被继承？</font>
...
<font color=blue size=5>A</font>   <font size=4>左值和右值的区别？左值引用和右值引用的区别，如何将左值转换成右值？</font>
...
<font color=blue size=5>A</font>   <font size=4>std::move() 函数的实现原理</font>
...
<font color=blue size=5>A</font>   <font size=4>什么是指针？指针的大小及用法？</font>
...
<font color=blue size=5>A</font>   <font size=4>什么是野指针和悬空指针？</font>
...
<font color=blue size=5>A</font>   <font size=4>C++ 11 nullptr 比 NULL 优势</font>
...
<font color=blue size=5>A</font>   <font size=4>指针和引用的区别？</font>
...
<font color=blue size=5>A</font>   <font size=4>常量指针和指针常量的区别</font>
...
<font color=blue size=5>A</font>   <font size=4>函数指针和指针函数的区别</font>
...
<font color=blue size=5>A</font>   <font size=4>强制类型转换有哪几种？</font>
...
<font color=blue size=5>A</font>   <font size=4>如何判断结构体是否相等？能否用 memcmp 函数判断结构体相等？</font>
...
<font color=blue size=5>A</font>   <font size=4>参数传递时，值传递、引用传递、指针传递的区别？</font>
...
<font color=blue size=5>A</font>   <font size=4>什么是模板？如何实现？</font>
...
- 函数模板实例化：当调用一个模板时，编译器用函数实参来推断模板实参
...
<font color=blue size=5>A</font>   <font size=4>函数模板和类模板的区别？</font>
...
<font color=blue size=5>A</font>   <font size=4>什么是可变参数模板？</font>
...
<font color=blue size=5>A</font>   <font size=4>什么是模板特化？为什么特化？</font>
...
<font color=blue size=5>A</font>   <font size=4>include " " 和 <> 的区别</font>
...
<font color=blue size=5>A</font>   <font size=4>switch 的 case 里为何不能定义变量</font>
...
<font color=blue size=5>A</font>   <font size=4>迭代器的作用？</font>
...
<font color=blue size=5>A</font>   <font size=4>泛型编程如何实现？</font>
...
<font color=blue size=5>A</font>   <font size=4>什么是类型萃取？</font>
...
<font size=4>设计模式</font>

***



<font color=blue size=5>A</font>   <font size=4>了解哪些设计模式？</font>
...
<font color=blue size=5>A</font>   <font size=4>什么是单例模式？如何实现？应用场景？</font>
...
<font color=blue size=5>A</font>   <font size=4>什么是工厂模式？如何实现？应用场景？</font>
...
<font color=blue size=5>A</font>   <font size=4>什么是观察者模式？如何实现？应用场景？</font>
...
该 LeetBook 参考《Effective C++》、《More Effective C++》、《Effective C++》、《Visual C++ 代码参考与技巧大全》、《STL》源码剖析、《大话设计模式》、《设计模式:可复用面向对象软件的基础》整理。
